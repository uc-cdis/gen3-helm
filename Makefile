...
local: deploy zip ## Deploy the Local commons
local-context: change-context # Change to the Local context
local local-context: DEPLOY=local
...
development development-context: DEPLOY=development
...
staging staging-context: DEPLOY=staging
...
production production-context: DEPLOY=production
...
change-context:
	@echo "Changing context to $(DEPLOY)"
...
deploy: check-context check-secrets
	@if [ ! -z $(CONFIRM_DEPLOY) ] && [ $(CONFIRM_DEPLOY) == "true" ]; then \
...
zip: 
	...
	@zip -q -r Secrets-$(TIMESTAMP).zip Secrets
	@echo "Archiving Secrets: Secrets-$(TIMESTAMP)"
	@mv Secrets-$(TIMESTAMP).zip $(HOME)/OneDrive/ACED-deployments
