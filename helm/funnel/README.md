...
| mongodb.readinessProbe.failureThreshold | int | `10` |  |
...
| postgresql | map | `{"enabled":false,"primary":{"persistence":{"enabled":false}}}` | Postgresql subchart settings if deployed separately option is set to "true". Disable persistence by default so we can spin up and down ephemeral environments |
