...
| externalSecrets | map | `{"createK8sJwtKeysSecret":false,"createK8sSsjSecret":false,"createK8sSsjdispatcherSecret":false,"credsFile":null}` | External secrets configuration |
...
| externalSecrets.createK8sSsjSecret | string | `false` | Will create the Helm "ssjdispatcher-creds"" secret even if Secrets Manager is enabled. This is helpful if you are wanting to use External Secrets for some, but not all secrets. |
