...
function getServiceReleases(req) {
  ...
  var release_cookie =
    req.variables["cookie_dev_canaries"] ||
    req.variables["cookie_service_releases"] ||
    "";
  ...
  var hash_str = [
    "app",
    req.variables["realip"],
    req.variables["http_user_agent"],
    req.variables["date_gmt"],
  ].join();
  var hash_res = -1;
  ...
  for (var i = 0; i < services.length; i++) {
    ...
    if (!devOverride && getWeight(service, canary_weights) === 0) {
      ...
      updated_releases[service] = selectRelease(
        hash_res,
        getWeight(service, canary_weights)
      );
    } else {
...
function isCredentialsAllowed(req) {
  if (!!req.variables["http_origin"]) {
    var origins =
      JSON.parse(req.variables["origins_allow_credentials"] || "[]") || [];
    for (var i = 0; i < origins.length; i++) {
      // cannot use === to compare byte strings, whose "typeof" is also confusingly "string"
      if (
        origins[i].fromUTF8().toLowerCase().trim() ===
